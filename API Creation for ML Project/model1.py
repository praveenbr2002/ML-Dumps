...
def crop_face_image(face_source_dir, mode):
    img = face_source_dir#cv2.imread(face_source_dir)
...
def crop_eye_image(eye_source_dir, mode):
    img = eye_source_dir
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    eyes = eye_cascade.detectMultiScale(gray)
    eye_startX, eye_startY, eye_width, eye_height= (0,0,0,0)
...
def crop_forehead_image(forehead_source_dir, mode):
    imgs = forehead_source_dir
    gray = cv2.cvtColor(imgs, cv2.COLOR_BGR2GRAY)
    eyes = eye_cascade.detectMultiScale(gray)
    eye_startX, eye_startY, eye_width, eye_height= (0,0,0,0)
    for (eye_startX, eye_startY, eye_width, eye_height) in eyes:
        print(eye_startX, eye_startY, eye_width, eye_height)
...
#Function to crop the Forehead Part
def crop_cheek_image(cheek_source_dir, mode):
    imgsc = cheek_source_dir
...
def mainfunction1(face_source_dir, mode=1):
    # Function call for face 
    
    w = crop_face_image(face_source_dir, mode)
    
    # Function Call for eye
    x = crop_eye_image(w, mode)
    
    # Function Call for forehead 
    y = crop_forehead_image(x, mode)
    
    # Function Call for cheeks
    z = crop_cheek_image(y, mode)
            
    ######### Dark Circle Prediction ########### Beolw Eye Part
    img_drk_crkl = x
    print(img_drk_crkl.shape)
